...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onNewChat}
        className="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-white/60 hover:bg-slate-100/80 text-slate-800 border border-slate-200/80 rounded-full font-semibold text-sm transition-all shadow-xs hover:shadow-sm mb-4"
      >
        <Plus size={16} />
        새로운 대화
      </button>
      ...
      <div className="mt-3 pt-3 border-t border-slate-200/50 space-y-2">
        <button
          ...
          className="flex items-center justify-center gap-2 w-full py-2 px-3 text-xs font-medium text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"
        >
          <Trash2 size={13} />
          모든 대화 내역 삭제
        </button>
